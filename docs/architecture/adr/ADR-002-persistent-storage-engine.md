...
- **Cons:**
  ...
  - No built-in replication. Acceptable: single-VPS deployment, PO accepts this per PRD §7.
...
- **Negative:** Single-writer serialization (mitigated by async write queue). No replication (accepted per PRD). No network access for external tools (PO uses bot commands).
