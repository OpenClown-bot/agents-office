...
- `src/smm_autopilot/scheduler/jobs.py` (APScheduler job registration: all periodic tasks from ARCH-001@0.1.0 §4)
...
- Publisher adapter implementations — belongs to TKT-006@0.1.0
- Approval queue bot — belongs to TKT-005@0.1.0
- Draft generation — belongs to TKT-004@0.1.0
- Analytics-driven optimal-times posting (PRD NG11) — explicitly excluded

## 4. Inputs (Executor MUST read before writing code)
- ARCH-001@0.1.0 §3.5 Scheduler (responsibility, inputs, outputs, failure modes)
- ARCH-001@0.1.0 §4 Data Flow (steps 5–7: schedule, publish, expire)
- ARCH-001@0.1.0 §5 Data Model (`PublishJob`, `Draft`, `Channel`, `CadenceConfig` schemas)
- ADR-004@0.1.0 (APScheduler 3.x with AsyncIOScheduler)
- TKT-001@0.1.0 outputs: `db.py`, `models.py`, `config.py`
- TKT-005@0.1.0 outputs: cadence config stored in DB by bot commands
- TKT-006@0.1.0 outputs: `publishers/base.py` interface
...
- Static time windows only — no analytics-driven scheduling (PRD NG11)
