...
component: "all"
depends_on: []
blocks: [TKT-002, TKT-003, TKT-004, TKT-005, TKT-006, TKT-007, TKT-008, TKT-009]
estimate: M
...
Create the Python project structure with async entrypoint, SQLite database initialization (WAL mode, all tables from ARCH-001@0.1.0 §5), Docker configuration with resource limits, and dependency pinning.
...
- `src/smm_autopilot/models.py` (dataclass definitions matching ARCH-001@0.1.0 §5 schemas)
...
- Any business logic (ingestion, classification, drafting, publishing) — belongs to TKT-002@0.1.0 through TKT-008@0.1.0
- Observability configuration beyond basic structlog setup — belongs to TKT-009@0.1.0
- Deployment scripts — belongs to TKT-009@0.1.0

## 4. Inputs (Executor MUST read before writing code)
- ARCH-001@0.1.0 §5 Data Model / Schemas (all table definitions)
- ARCH-001@0.1.0 §10 Deployment (Docker resource limits, container structure)
...
- [ ] All tables from ARCH-001@0.1.0 §5 exist with correct columns
...
- Do NOT add dependencies beyond: `aiosqlite>=0.20.0`, `httpx>=0.27.0`, `python-telegram-bot>=21.0`, `apscheduler>=3.10,<4.0`, `structlog>=24.0`, `feedparser>=6.0`, `beautifulsoup4>=4.12`, `lxml>=5.0`, `pydantic>=2.0` (for config validation)
- Do NOT use SQLAlchemy ORM — use raw SQL via aiosqlite for simplicity and control
