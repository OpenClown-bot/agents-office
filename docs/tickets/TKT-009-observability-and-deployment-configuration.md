...
Configure structured JSON logging via structlog across all components, implement the `/stats` bot command metrics query, create the deploy script with pre-deploy DB backup, and set up the systemd slice for OS-level resource ceilings.

## 2. In Scope
- `src/smm_autopilot/logging.py` (structlog JSON configuration: timestamp, level, component, event, trace_id fields)
- `src/smm_autopilot/bot/commands/stats.py` updates (query `metrics` and `publish_log` tables, format daily/weekly summary)
...
- Prometheus/Grafana setup — explicitly excluded per ARCH-001@0.1.0 §8 (no dev-ops rotation)
- Application business logic — belongs to TKT-002@0.1.0 through TKT-008@0.1.0
- CI/CD pipeline — out of scope for MVP

## 4. Inputs (Executor MUST read before writing code)
- ARCH-001@0.1.0 §8 Observability (log format, metrics, alerting, tracing)
- ARCH-001@0.1.0 §9 Security (secrets management, no secrets in git)
- ARCH-001@0.1.0 §10 Deployment (rollback procedure, backup, systemd slice, Docker log rotation)
- TKT-001@0.1.0 outputs: `docker-compose.yml`, `db.py`

## 5. Outputs (deliverables)
- [ ] `src/smm_autopilot/logging.py`
- [ ] Updated `src/smm_autopilot/bot/commands/stats.py`
- [ ] `infra/deploy.sh`
...
- Do NOT add new dependencies beyond those in TKT-001@0.1.0's `requirements.txt` (structlog is already included)
- Do NOT create `.env` files — only reference `.env.example` from TKT-001@0.1.0
