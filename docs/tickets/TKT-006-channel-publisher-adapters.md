...
- Scheduling logic (when to publish) — belongs to TKT-007@0.1.1
- Approval queue — belongs to TKT&#45;005a@0.1.0
...
- [ ] Given a transient platform failure remains after adapter-local retries, when `publish()` returns, then the result includes retryable failure metadata for Scheduler re-queue instead of silently dropping the job
