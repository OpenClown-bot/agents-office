...
1. Generate a new file `docs/session-log/YYYY-MM-DD-session-N.md` based on `docs/session-log/TEMPLATES/handoff-cold-devin.md`.
2. Fill in the State, Decisions, Next planned action, and Pitfalls sections from the just-closed cycle.
3. Commit + push (via PR if branch protection requires it; orchestrator's `docs/session-log/` write-zone allows direct-to-main commits when no other artifact is touched).
4. Reference the handoff file in its next message to the PO ("cold handoff written: `<path>`").
