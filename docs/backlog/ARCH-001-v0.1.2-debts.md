...
## 8. From RV-CODE-004 (Kimi K2.6 CODE review of TKT-004 PR#17, verdict fail)

Findings F-1, F-2, F-3, F-4, F-6, F-7 are addressed in the same PR cycle (Architect mini-cycle for F-4 schema change; Executor follow-up commit for the rest). F-5 and F-8 are ticketed here as low-priority follow-ups.

### Low
- **F-5 (LOW) — Prompt-injection adversarial test only verifies prompt structure, not LLM behavioral resilience.** `tests/test_drafting.py:test_prompt_injection_mitigation` builds the escaped prompt and asserts line structure, then parses a hardcoded JSON string — never submits the adversarial prompt to an LLM and never verifies that injected instructions are ignored. AC6 claims "no injected instruction is followed" but the test only proves escaping mechanics. **Remediation**: add a mocked `LLMClient` test that asserts the response parser rejects non-JSON output or output containing the injected instruction text; if real LLM calls are unavailable, document the limitation in the test docstring. **Owner**: follow-up TKT or addressed when QA-PLAYBOOK adds an LLM-behavioral test harness.
- **F-8 (LOW) — `generate_drafts` does not implement queue backpressure limits from ARCH-001@0.1.2 §4.1.** `service.py:84` fetches all `classified` items without checking whether the `drafts` table backlog exceeds 500 rows. The ArchSpec states DraftGenerator should "pause non-time-sensitive generation and alerts PO" when `drafts(status=ready|unverified|deferred) >= 500`. This is not an AC but is a contract divergence. **Remediation**: add a pre-flight count query against the `draft` table; if non-time-sensitive items exceed the threshold, skip generation and log a warning with `structlog`. **Owner**: follow-up TKT or addressed when ARCH-001@0.1.3 finalizes backpressure semantics (already noted as deferred item under §1.2 Queue backpressure undefined).
